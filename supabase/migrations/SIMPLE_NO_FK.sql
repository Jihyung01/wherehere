-- ============================================================
-- WhereHere 실제 DB 스키마 (Foreign Key 없는 버전)
...
('kakao-place-004', '홍대 카페', '서울 마포구 홍대입구', ST_SetSRID(ST_MakePoint(126.9250, 37.5563), 4326)::geography, '카페', ARRAY['활기찬', '젊은', '음악'], '홍대의 활기찬 카페 거리', 4.4, false, 11000, 'high'),
('kakao-place-005', '강남 카페', '서울 강남구 신사동', ST_SetSRID(ST_MakePoint(127.0205, 37.5172), 4326)::geography, '카페', ARRAY['세련된', '고급', '비즈니스'], '강남의 세련된 카페', 4.5, false, 18000, 'medium'),

-- 공원
('kakao-place-006', '한강공원', '서울 영등포구 여의도동', ST_SetSRID(ST_MakePoint(126.9329, 37.5285), 4326)::geography, '공원', ARRAY['자연', '운동', '산책'], '한강을 따라 산책하기 좋은 공원', 4.8, false, 0, 'high'),
('kakao-place-007', '서울숲', '서울 성동구 성수동1가', ST_SetSRID(ST_MakePoint(127.0374, 37.5445), 4326)::geography, '공원', ARRAY['자연', '가족', '반려동물'], '도심 속 자연을 만끽할 수 있는 공원', 4.7, false, 0, 'medium'),
('kakao-place-008', '남산공원', '서울 용산구 용산동2가', ST_SetSRID(ST_MakePoint(126.9910, 37.5512), 4326)::geography, '공원', ARRAY['자연', '등산', '야경'], '서울 시내를 한눈에 볼 수 있는 공원', 4.6, false, 0, 'high'),

-- 문화공간
('kakao-place-009', '북촌 한옥마을', '서울 종로구 계동길', ST_SetSRID(ST_MakePoint(126.9850, 37.5820), 4326)::geography, '문화공간', ARRAY['전통', '사진', '관광'], '전통 한옥의 아름다움을 느낄 수 있는 곳', 4.6, false, 0, 'high'),
('kakao-place-010', '삼청동 갤러리', '서울 종로구 삼청동', ST_SetSRID(ST_MakePoint(126.9820, 37.5860), 4326)::geography, '문화공간', ARRAY['예술', '갤러리', '감성'], '작은 갤러리들이 모여있는 거리', 4.5, true, 0, 'low'),
('kakao-place-011', '국립중앙박물관', '서울 용산구 서빙고로', ST_SetSRID(ST_MakePoint(126.9800, 37.5240), 4326)::geography, '박물관', ARRAY['문화', '역사', '교육'], '한국의 역사와 문화를 한눈에', 4.9, false, 0, 'medium'),

-- 맛집
('kakao-place-012', '익선동 맛집', '서울 종로구 익선동', ST_SetSRID(ST_MakePoint(126.9900, 37.5710), 4326)::geography, '음식점', ARRAY['한식', '전통', '맛집'], '전통 한옥에서 즐기는 현대식 한식', 4.7, true, 30000, 'high'),
('kakao-place-013', '망원동 맛집', '서울 마포구 망원동', ST_SetSRID(ST_MakePoint(126.9050, 37.5560), 4326)::geography, '음식점', ARRAY['다양한', '저렴한', '로컬'], '다양한 음식을 저렴하게 즐길 수 있는 곳', 4.4, false, 15000, 'medium'),
('kakao-place-014', '이태원 맛집', '서울 용산구 이태원동', ST_SetSRID(ST_MakePoint(126.9942, 37.5347), 4326)::geography, '음식점', ARRAY['이국적', '다양한', '세계음식'], '세계 각국의 음식을 맛볼 수 있는 곳', 4.5, false, 35000, 'high'),

-- 야경/뷰
('kakao-place-015', 'N서울타워', '서울 용산구 남산공원길', ST_SetSRID(ST_MakePoint(126.9882, 37.5512), 4326)::geography, '전망대', ARRAY['야경', '데이트', '관광'], '서울의 야경을 한눈에 볼 수 있는 곳', 4.8, false, 15000, 'high')

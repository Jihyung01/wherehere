-- WhereHere Extended Schema (No Foreign Keys Version)
-- Create tables first, add foreign keys later if needed
...
CREATE INDEX IF NOT EXISTS idx_completed_quests_user ON completed_quests(user_id, completed_at DESC);
CREATE INDEX IF NOT EXISTS idx_completed_quests_place ON completed_quests(place_id);
...
CREATE INDEX IF NOT EXISTS idx_gatherings_place_time ON gatherings(place_id, scheduled_time);
CREATE INDEX IF NOT EXISTS idx_gatherings_status ON gatherings(status, scheduled_time);
...
-- Function: Update user stats (without foreign key constraint)
...
DROP TRIGGER IF EXISTS trigger_update_user_stats ON completed_quests;

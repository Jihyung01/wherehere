...
export type MyMapScreenProps = {
  /** 앱 내 프로필·지도 탭에 임베드 시 true. 하단 네비 숨김, 도메인 이탈 없음 */
  isEmbedded?: boolean;
  /** 테마 (미전달 시 localStorage 사용) */
  isDarkMode?: boolean;
  /** 임베드 시 "첫 탐험 시작하기" 등 홈 이동 시 호출 (리로드 대신 화면 전환) */
  onGoHome?: () => void;
};

// Main Component
export function MyMapScreen({ isEmbedded = false, isDarkMode: isDarkModeProp, onGoHome }: MyMapScreenProps = {}) {
  ...
  const isDarkMode = isDarkModeProp ?? isDarkModeLocal;
  ...
  // Load theme (임베드가 아닐 때만 localStorage 반영)
  useEffect(() => {
    if (isDarkModeProp !== undefined) return;
    const savedTheme = localStorage.getItem('isDarkMode');
    if (savedTheme) setIsDarkModeLocal(savedTheme === 'true');
  }, [isDarkModeProp]);
  ...
  return (
    <>
      ...
      <div style={{ maxWidth: 430, margin: "0 auto", minHeight: "100vh", background: bgColor, color: textColor, fontFamily: 'Pretendard, sans-serif', position: "relative", overflow: "hidden" }}>
              ...
              <div style={{ position: "relative", width: "100%", marginBottom: 16 }}>
                ...
                {visits.length === 0 && (
                  ...
                  >
                    ...
                    <button
                      type="button"
                      onClick={() => { onGoHome?.(); if (!onGoHome) router.push("/"); }}
                      style={{
                        padding: "10px 20px",
                        borderRadius: 10,
                        border: "none",
        ...
        {/* Bottom Nav — 임베드 시 숨김 (앱 내에서는 상단 탭으로 이동) */}
        {!isEmbedded && (
